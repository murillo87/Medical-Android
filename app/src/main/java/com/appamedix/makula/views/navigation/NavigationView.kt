...
/* The custom view class for the navigation. */
class NavigationView(context: Context, attrs: AttributeSet) : CardView(context, attrs) {

    // The UI elements to consist this view.
    private var titleTextView: TextView
    private var leftButton: ImageButton
    private var rightButton: ImageButton
    private var separator: View

    // The listener for the left and right buttons.
    private var listener: NavigationViewListener?
    ...
    init {
        ...
        val view = inflater.inflate(R.layout.navigation_view, this, true)

        view.setBackgroundColor(ContextCompat.getColor(context, R.color.darkMain))
        titleTextView = view.findViewById(R.id.nav_title)
        leftButton = view.findViewById(R.id.nav_left_button)
        rightButton = view.findViewById(R.id.nav_right_button)
        separator = view.findViewById(R.id.nav_cell_separator)
        listener = null
    }
     ...
     */
    fun setupView(model: NavigationCellViewModel) {
        // Apply view model.
        ...
        leftButton.setOnClickListener { _ -> listener?.leftButtonClicked() }
        rightButton.setOnClickListener { _ -> listener?.rightButtonClicked() }
        ...
        // Adjust the view elements in the case of large style, e.g. landscape mode.
        if (model.largeStyle) {
            // Enlarge the text size of title view.
            titleTextView.textSize = Const.Font.titleLarge
